...
const Bookings = () => {
  ...
  const handleCancelBooking = async (booking) => {
    try {
      ...
      setLoading(true);
      await bookingApi.cancelBooking({ bookingId: booking._id });
  ...
  const handleRecheduleSubmit = useCallback(
    async (values) => {
      setLoading(true);
      try {
        ...
        const hasConflicts = conflictResponses.some(
  ...
  const columns = [
...

    {
      ...
      render: (text, record) =>
        ...
        record.status === "reschedulerequest" ? null : (
...

    {
      ...
      render: (user, record) => (
        <AiOutlineEye
          ...
          style={{
            color:
              record?.service?.courseType === "fixed-course"
                ? "#ccc"
                : "#1890ff",
            cursor:
              record?.service?.courseType === "fixed-course"
                ...
                : "pointer",
            pointerEvents:
              record?.service?.courseType === "fixed-course" ? "none" : "auto",
          }}
        />
      ),
    },
  ...
  if (user.role === "mentor" || user.role === "student") {
    columns.push({
      ...
      render: (_, record) => {
        ...
        const isStudent =
          user.role === "student" && record.status === "reschedulerequest";
        ...
        if (isMentor || isStudent) {
          const dropdownMenu = (
            <Menu>
              {isMentor && record.serviceType === "one-on-one" && (
                <Menu.Item
                  key="reschedule"
    ...
    <>
      <div className="p-6 mx-auto bg-white rounded-lg shadow-lg h-full m-8 lg:mt-20">
        ...
        <Modal
          ...
          destroyOnClose
        >
          <Form form={form} onFinish={handleRecheduleSubmit} layout="vertical">
            <Form.List name="availability">
              {(fields, { add, remove }) => (
                <>
                  {fields.map(({ key, name, ...restField }) => (
                    <div key={key} className="flex gap-2 mb-2 items-end">
                      <Form.Item
                        {...restField}
                        name={[name, "date"]}
                        label="Date"
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-md animate-fadeIn">
            <div className="bg-white p-3 lg:p-6 top-20 rounded-2xl shadow-2xl w-64 lg:w-96 border border-gray-200 relative">
              <div className="my-2 lg:my-5">
                <h3 className="text-lg lg:text-2xl font-semibold mb-3 text-gray-900">
                  User Details
                </h3>
                <p className="text-gray-700 text-sm lg:text-md">
                  <strong>Full name:</strong> {selectedBooking?.user?.name}
                </p>
                <p className="text-gray-700 text-sm lg:text-md">
                  <strong>Email:</strong> {selectedBooking?.user?.email}
                </p>
                <p className="text-gray-700 text-sm lg:text-md">
                  <strong>Username:</strong> {selectedBooking?.user?.username}
                </p>
              </div>
